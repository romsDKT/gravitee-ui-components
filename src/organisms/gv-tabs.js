 ...
 *
 * @attr {Array<id: string | {id, title?, icon?, active?, description?}>} options - An array of options for gv-option sub component
    ...
    if (this.truncate) {
      return this.options.map((option) => {
        if (this.value !== option.id) {
          return { ...option, label: `${option.title.substring(0, 1)}..` };
        }
        return option;
      });
    }
